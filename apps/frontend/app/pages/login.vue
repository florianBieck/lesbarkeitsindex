    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-col items-center gap-4">
        ...
        <div class="flex flex-col items-center gap-2 w-full">
          <div class="text-surface-900 dark:text-surface-0 text-2xl font-semibold leading-tight text-center w-full">Anmelden</div>
        </div>
      </div>
      <div class="flex flex-col gap-6 w-full">
        <div class="flex flex-col gap-2 w-full">
          <label for="email1" class="text-surface-900 dark:text-surface-0 font-medium leading-normal">E-Mail Adresse</label>
          <InputText id="email1" v-model="email" fluid type="text" placeholder="Email address" class="px-3 py-2 shadow-sm rounded-lg" />
        </div>
        <div class="flex flex-col gap-2 w-full">
          <label for="password1" class="text-surface-900 dark:text-surface-0 font-medium leading-normal">Passwort</label>

          <Password id="password1" v-model="password" fluid placeholder="Password" :toggleMask="true" :feedback="false" />
        </div>
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-3 sm:gap-0">
          <div class="flex items-center gap-2">
            ...
            <label for="rememberme1" class="text-surface-900 dark:text-surface-0 leading-normal">Anmeldung speichern?</label>
          </div>
          <a class="text-primary font-medium cursor-pointer hover:text-primary-emphasis">Passwort vergessen?</a>
        </div>
      </div>
      <Button @click="login" label="Sign In" icon="pi pi-user" fluid class="w-full py-2 rounded-lg flex justify-center items-center gap-2">
        <template #icon>
